...
class DeltalakeStorageManager:
    ...
    def __init__(
        self, storage_dir: str, num_processes: int = 0, write_batch_size: int = 10000
    ):
    ...
    def read_split(
        ...
        if streaming_mode:
            base_iterator = DeltalakeLocalStreamReader(  # type: ignore
                path=str(self.split_dir(split=split)),
                data_model=data_model,
                allowed_keys=allowed_keys,
            )
        else:
            base_iterator = DeltalakeInMemoryReader(
                path=str(self.split_dir(split=split)),
                data_model=data_model,
                allowed_keys=allowed_keys,
            )
