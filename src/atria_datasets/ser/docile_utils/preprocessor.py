...
_DOCILE_CLASSES = (
    ["KILE_background", "LI_background", "LIR_background"]
    + KILE_FIELDTYPES
    + LIR_FIELDTYPES
)
...
def generate_unique_entities():
    ...
    unique_entities.extend(
        [f"O-{x.rstrip('_background')}" for x in _DOCILE_CLASSES if "background" in x]
    )

    # add KILE and LIR class tags
    unique_entities.extend([f"B-{x}" for x in _DOCILE_CLASSES if "background" not in x])
    unique_entities.extend([f"I-{x}" for x in _DOCILE_CLASSES if "background" not in x])
...
def tag_fields_with_entities(fields, unique_entities=[]):  # noqa: B006
    ...
    if len(unique_entities) < 1:
        entity_map = dict.fromkeys(_DOCILE_CLASSES, False)
    else:
...
def get_data_from_docile(dataset, overlap_thr=0.5, image_shape=(1024, 1024)):
    data = []
    metadata = []

    for document in tqdm(dataset, desc=f"Generating data from {dataset}"):
        sample_id = document.docid
        ...
        for page in range(document.page_count):
            ...
            kile_li_fields_page_sorted = sorted(
                enumerate(kile_li_fields_page), key=lambda i_f: i_f[1].bbox.top
            )
            fields_top_coords = [
            ...
            for i in range(1, len(kile_li_fields_page_sorted)):
                fields_bottom_coords_max[i] = max(
                    fields_bottom_coords_max[i], fields_bottom_coords_max[i - 1]
                )
...
class DOCILEDataLoader:
    def __init__(
        self, data, metadata, unique_entities=None, use_BIO_format=True
    ) -> None:
        ...
        for page_data, page_metadata in tqdm(
            ...
            tokens_with_entities = tag_fields_with_entities(
                page_data, self.unique_entities
            )
        ...
        for tokens_with_entities in tqdm(
            temp_processed_tables, desc="Processing tables 2/2"
        ):
...
def prepare_docile_dataset(
    ...
    if (
        ...
        return
    logger.info(
        f"Could not load preprocessed {docile_dataset.split_name}, regenerating and storing data to {preprocessed_dataset_path}."
    )
    data, metadata = get_data_from_docile(
        docile_dataset, overlap_thr=overlap_thr, image_shape=image_shape
    )
...
def load_docile_dataset(
    docile_dataset, preprocessed_dataset_path, image_shape=(1024, 1024)
):
    dataset_name = docile_dataset.data_paths.name
    preprocessed_path = preprocessed_dataset_path / dataset_name
    logger.info(
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not load preprocessed dataset due to following exception: {e}"
        )
    return DOCILEDataLoader(data, metadata, unique_entities=generate_unique_entities())
