...
class DeltaStreamer(Sequence[T_BaseDataInstance]):
    ...
    def __getitem__(self, index: int) -> T_BaseDataInstance | list[T_BaseDataInstance]:
        self._initialize_dataset()
        self._initialize_s3_client()
        rows = self._pa_dataset.take([index]).to_pylist()
    ...
    def _process_rows(self, rows: list[dict]) -> list[T_BaseDataInstance]:
        ...
        for row in rows:
            ...
            row["image_file_path"] = "metadata.yaml"
...
streamer = DeltaStreamer(path, ImageInstance, storage_options=storage_options)
print(streamer)
print(f"Total samples: {len(streamer)}")
for sample in streamer[0]:
    print(sample)
