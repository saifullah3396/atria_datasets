        ...
        try:
            ...
            if config and "credentials" in config:
                ...
                if "secret_access_key" in config["credentials"]:
                    result["secret_access_key"] = config["credentials"][
                        "secret_access_key"
                    ]
...
def dump_repository(client, repo_name, commit=False, delete_after_dump=False):
    ...
    try:
        ...
        if commit:
            ...
            for branch in paginate_results(
                client.branches_api.list_branches, repository=repo_name
            ):
                branch_id = branch.id
                # Get uncommitted changes
                uncommitted = client.branches_api.diff_branch(
                    repository=repo_name, branch=branch_id
                )

                # If there are uncommitted changes, commit them
                if uncommitted.results:
                    print(
                        f"Found {len(uncommitted.results)} uncommitted changes in branch {branch_id}"
                    )
        ...
        print("Dumping repository metadata...")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="lakeFS Repository References Management"
    )
    ...
    dump_group.add_argument(
        "repository", nargs="?", help="Name of the repository to dump"
    )
    dump_group.add_argument("--all", action="store_true", help="Dump all repositories")
    dump_parser.add_argument(
        "--commit",
        action="store_true",
        help="Commit uncommitted changes before dumping",
    )
    dump_parser.add_argument(
        "--rm", action="store_true", help="Delete repository after successful dump"
    )

    # Restore command
    restore_parser = subparsers.add_parser(
        "restore", help="Restore repository references"
    )
    restore_parser.add_argument(
        "manifests", nargs="+", help="One or more manifest files to restore"
    )
    restore_parser.add_argument(
        "--ignore-storage-id",
        action="store_true",
        help="Create repository without storage_id",
    )
    ...
    if args.command == "dump":
        if args.all:
            success = dump_all_repositories(
                client=client, commit=args.commit, delete_after_dump=args.rm
            )
        else:
